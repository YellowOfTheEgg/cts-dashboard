...
const fileUploadEndpoint = "clustering/moscat/dataset";
const fileSettingsEndpoint = "clustering/moscat/settings-dataset";
...
export const FileUpload = () => {
  ...
  const { setShowClustering, setShowResultCard } =
    React.useContext(MoscatContext);

  const onSeparatorChange = (e: React.ChangeEvent<HTMLInputElement>) =>
    setColumnSeparator(e.target.value);
  const onObjectIdColumnChange = (e: React.ChangeEvent<HTMLInputElement>) =>
    setObjectIdColumn(e.target.value);
  const onTimeColumnChange = (e: React.ChangeEvent<HTMLInputElement>) =>
    setTimeColumn(e.target.value);
  const onFeatureColumnsChange = (e: React.ChangeEvent<HTMLInputElement>) =>
  ...
  function dtypesMatching() {
    ...
    if (objectIdColumn === "" || timeColumn === "" || featureColumns === "") {
  ...
  const handleUploadDataset = () => {
    ...
    gatewayApi
      .post(fileSettingsEndpoint, settingsDataset)
      .then((outerResponse) => {
        ...
        gatewayApi
          .post(fileUploadEndpoint, formData, { headers: headers })
          .then((innerResponse) => {
            // set this variable for the user to see the result of the request
            setDatasetRequestResult({
              requestDone: true,
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} sm={12} md={4}>
            <TextField
              ...
              onChange={onObjectIdColumnChange}
