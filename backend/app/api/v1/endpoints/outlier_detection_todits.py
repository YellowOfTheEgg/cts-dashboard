...
def set_settings_todits(settingsTodits: SettingsTodits, request: Request):
    state = request.app.state.todits
    session = request.cookies.get("session")
    store_data(session, state, "settings_todits", settingsTodits)
...
def run(request: Request):
    ...
    todits = Todits(dataset_df, settings_dataset, settings_todits)
...
def reset(request: Request):
    session = request.cookies.get("session")
    delete_data(session, request.app.state.todits)
