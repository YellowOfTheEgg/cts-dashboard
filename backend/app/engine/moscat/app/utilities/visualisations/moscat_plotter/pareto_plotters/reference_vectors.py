...
class ReferenceVectors(IParetoPlotter):
    ...
    def add_plot_settings(self):
        ...
        plt.xlim(-0.1, self.moscat.weighting_system.max_snapshot_quality + 0.1)
        plt.ylim(-0.1, self.moscat.weighting_system.max_temporal_quality + 0.1)
    ...
    def add_scatter_legend(
        ...
        clustering_parameter_keys = intermediate_step["tpcs_properties"][
            0
    ...
    def create_plot(self, img_path):
        ...
        for step_idx, logged_tpc_properties in enumerate(
            tqdm(self.moscat.logged_tpcs_properties, desc="Creating plots")
        ):

            self.add_guide_lines()
            handles, labels = self.add_scores(
                logged_tpc_properties["tpcs_properties"],
                logged_tpc_properties["pareto_front"],
                self.moscat.selected_tpcs_properties[step_idx],
            )

            self.add_plot_settings()
            legend_2 = self.add_general_legend()
            legend_1 = self.add_scatter_legend(
                self.moscat.logged_tpcs_properties[0],
                handles,
                labels,
                legend_denominator,
            )
