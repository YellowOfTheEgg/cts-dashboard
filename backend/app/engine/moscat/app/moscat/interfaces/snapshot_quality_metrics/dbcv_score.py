...
class DbcvScore(IQualityMetric):
    def __init__(self):
    ...
    def calculate_score(self, tpc):
        ...
        if len(noiseless_clustering) > 0:
            X = np.array([list(f) for f in noiseless_clustering[self.col_features]])
            N = np.array([list(f) for f in noise[self.col_features]])
