...
class FCSETS(object):
    ...
    def calc_sequence_stability(
        ...
        hr_dict = self.calculate_rel_ass_agreement(
            start_time, end_time, num_time_series
        )
        ...
        for l in range(0, num_time_series):
            ...
            for i in range(start_time, end_time):
                for r in range(i + 1, end_time + 1):
                    ...
                    for s in range(0, num_time_series):
                        counter = counter + math.pow(
                            hr_dict[i][l][s], num_time_series
            ...
            stability_dict[l] = (
                1
                - (2 / (self.num_timestamps * (self.num_timestamps - 1)))
                * stability_dict[l]
            )
        return stability_dict

    def calculate_rel_ass_agreement(
        self, start_time: int, end_time: int, num_time_series: int
    ) -> dict:
        """
        Params:
            start_time (int) - time that should be considered as beginning
