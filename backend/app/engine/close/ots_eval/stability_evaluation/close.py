...
class CLOSE(object):
    ...
    def __init__(
        ...
        measure: Union[str, callable] = "mse",
        minPts: int = None,
        output: bool = False,
        jaccard: bool = False,
        weighting: bool = False,
        exploitation_term: bool = False,
    ):
        """
        Params:
    ...
    def rate_clustering(
        ...
        for cluster in cluster_ratings:
            cluster_objects = gr_clusters.get_group(cluster)[
                self._object_column_name
            ].unique()
            cluster_time = gr_clusters.get_group(cluster)[self._time_column_name].iloc[
                0
            ]
        ...
        if self._exp_term:
            ...
            factor = (
                (1 / num_clusters)
                * (1 - (num_timestamps / num_clusters) ** 2)
                * exp_term
            )
        else:
        ...
        if not return_measures:
            ...
            return {
                "stability_evaluation": score * factor,
                "stability": avg_stab,
                "quality": avg_quality,
                "pre-factor": (1 - (num_timestamps / num_clusters) ** 2),
            }

    def rate_time_clustering(
        self,
        start_time: int = None,
        end_time: int = None,
        return_measures: bool = False,
        ...
        """
        cluster_ratings = self.rate_clusters(start_time, end_time)
        num_timestamps, timestamps = self.get_num_timestamps(
            start_time, end_time, return_timestamps=True
        )
        ...
        for time in timestamps:
            if not return_measures:
                ...
                cur_scores = self.calc_t_clustering_rating(
                    cluster_ratings, time, return_measures=True
                )
    ...
    def calc_t_clustering_rating(
        self, cluster_ratings: dict, time: int, return_measures: bool = False
    ) -> Union[float, dict]:
        """
        Params:
        ...
        clusters_at_time = self._data[self._data[self._time_column_name] == time][
    ...
    def rate_clusters(
        ...
        """
        ids_to_rate = self.get_ids_to_rate(
            id, self._cluster_column_name, start_time, end_time
        )
        ids = ids_to_rate[:]
    ...
    def calc_cluster_rating(
        self, ids_to_rate: Union[list, np.ndarray], start_time: int = None
    ) -> dict:
        """
        Params:
        ...
        for id in ids_to_rate:
            ...
            if time != start_time:
                ...
                obj_list = (
                    gr_clusters.get_group(id)[self._object_column_name]
                    .unique()
                    .tolist()
                )
                obj_ratings = self.calc_object_rating(
                    cluster_compositions, obj_list, time
                )
                score = 0
                ...
                num_timestamps = len(
                    self._data.loc[
                        self._data[self._cluster_column_name].isin(clusters)
                ...
                try:
                    ...
                    if self._output:
                        print(
                            "<<ZeroDivisionError - Cluster Score>> Cluster ID: ",
                            str(id),
                            "  Merged Clusters: ",
                            str(num_merged_clusters),
                            "  Num Timestamps: ",
                            str(num_timestamps),
                        )
                    else:
    ...
    def rate_object(
        ...
        cluster_compositions = self.obtain_cluster_compositions()
        ratings = self.calc_object_rating(
            cluster_compositions, ids_to_rate, end_time, start_time
        )
        return ratings

    def calc_object_rating(
        self,
        cluster_composition: dict,
        ids_to_rate: Union[list, np.ndarray],
        end_time: int,
        start_time: int = None,
    ) -> dict:
        """
        ...
        for id in ids_to_rate:
            ...
            try:
                ...
                last_cluster = cur_group[cur_group[self._time_column_name] == end_time][
                    self._cluster_column_name
                ].iloc[0]
            except IndexError:
                print(
                    ">>INDEXERROR - LAST CLUSTER<< ID: ",
                    str(id),
                    ", Start Time: ",
                    str(start_time),
                    ", End Time: ",
            ...
            for cluster in cluster_ids:
                ...
                # Add the proportion of clusters before last timestamp, that merged in last cluster
                    ...
                    if int(cluster) < 0:
                        ...
                        object_ratings.append(
                            cluster_composition[last_cluster][cluster]
                        )
                    num_clusters += 1
                ...
                try:
                    ...
                    if num_clusters > 0:
                        ...
                        for i in range(num_clusters):
                            object_rating += object_ratings[i] * (
                                (i + 1) / weighting_denominator
                            )
    ...
    def calc_exploit(self) -> float:
        ...
        num_no_outliers = len(
            self._data[self._data[self._cluster_column_name] >= 0][
                self._object_column_name
            ].unique()
        )
    ...
    def get_feature_list(
        self, objects: Union[list, np.ndarray], time: int
    ) -> np.ndarray:
        """
        Params:
        ...
        for obj in objects:
            features = self._data[
                (self._data[self._object_column_name] == obj)
                & (self._data[self._time_column_name] == time)
            ]
            try:
                features = (
                    features.drop(
    ...
    def get_num_timestamps(
        self, start_time: int, end_time: int, return_timestamps: bool = False
    ) -> int:
        """
        Params:
    ...
    def obtain_cluster_compositions(self) -> dict:
        ...
        g_clusters = self._data.groupby(
            [self._time_column_name, self._cluster_column_name]
        )
        ...
        for group, objects in g_clusters:
            ...
            temp_intersection = (
                (
                    self._data.loc[
                        (self._data[self._object_column_name].isin(objects))
                        & (self._data[self._time_column_name] > group[0])
                    ]
                )
                .groupby(self._cluster_column_name)
                .count()
            )
            ...
            for cluster, num_objects in temp_intersection.iterrows():
                ...
                if self._jaccard:
                    ...
                    card_union = len(
                        self._data.loc[
                            (self._data[self._cluster_column_name] == cluster)
                            | (self._data[self._cluster_column_name] == group[1])
                        ][self._object_column_name].unique()
                    )
                    # jaccard distance
                    cluster_compositions[cluster][group[1]] = round(
                        float(num_objects.values[1]) / float(card_union), 3
                    )
                else:
                    cluster_compositions[cluster][group[1]] = round(
                        float(num_objects.values[1])
                        / float(cluster_members.loc[group[1]].values[1]),
                        3,
                    )
    ...
    def calc_max_dist(feature_list: list) -> float:
        ...
        for i in range(len(feature_list) - 1):
            for j in range(i + 1, len(feature_list)):
                cur_dist = euclidean(
                    np.array(feature_list[i]), np.array(feature_list[j])
                )
    ...
    def calc_exploit_at_t(self, time: int) -> float:
        ...
        """
        num_objects_at_t = len(
            self._data[self._data[self._time_column_name] == time][
                self._object_column_name
            ].unique()
        )
        num_no_outliers = len(
