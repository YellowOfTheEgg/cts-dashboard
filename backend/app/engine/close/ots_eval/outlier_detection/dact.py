...
class DACT(object):
    ...
    def calc_subsequence_ratings(self, start_time: int, end_time: int) -> pd.DataFrame:
        ...
        """
        subsequence_ratings = pd.DataFrame(
            columns=[self._object_column_name, "start_time", "end_time", "rating"]
        )
        ...
        for i in range(self._num_objects):
            ...
            num_timestamps = len(
                self._data[
                    (self._data[self._object_column_name] == self._object_ids[i])
            ...
            subsequence_ratings = subsequence_ratings.append(
                {
                    self._object_column_name: str(self._object_ids[i]),
    ...
    def calc_outlier_degree(self) -> pd.DataFrame:
        ...
        for t1 in range(len(timestamps) - 1):
            for t2 in range(t1 + 1, len(timestamps)):
                subsequence_ratings = self.calc_subsequence_ratings(
                    timestamps[t1], timestamps[t2]
                )
                subsequence_ratings = subsequence_ratings.assign(distance=0)
                relevant_data = self._data[
                    self._data[self._time_column_name] == timestamps[t2]
                ]
                for cid in relevant_data[self._cluster_column_name].unique():
                    if cid >= 0:
                        ...
                        best_score = np.amax(
                            subsequence_ratings[
                                subsequence_ratings[self._object_column_name].isin(
                                    relevant_ids
                                )
                            ]["rating"].values
                        )
        ...
        noise_object_ids = self._data[self._data[self._cluster_column_name] < 0][
            self._object_column_name
        ].unique()
        for object_id in noise_object_ids:
            noisy_timestamps = (
                self._data[
                    (self._data[self._object_column_name] == object_id)
            ...
            for i in range(1, len(noisy_timestamps)):
                if noisy_timestamps[i] == timestamps[start_index + counter]:
                    ...
                    if counter > 1:
                        self._outlier_rating = self._outlier_rating.append(
                            {
                                self._object_column_name: object_id,
    ...
    def mark_outliers(
        self, tau: float = None, percent: float = None
        ...
        if percent is not None:
            ...
            used_tau = 0.4
        self._outlier_result = self._outlier_rating[
            (
                (self._outlier_rating["distance"] >= used_tau)
        ...
        self._outlier_result = self._outlier_result.astype(
            {self._object_column_name: str}
        )

        for index, row in self._outlier_result.iterrows():
            for time_point in time_points[
                time_points.index(int(row["start_time"])) : time_points.index(
                    int(row["end_time"])
                )
                + 1
            ]:
                if row["distance"] == -1:
                    self._data.loc[
                        (self._data[self._time_column_name] == time_point)
                        & (
                            self._data[self._object_column_name]
                            == row[self._object_column_name]
                        ),
                        "outlier",
                    ] = -2
                else:
    ...
    def calc_statistic_cluster_outliers(
        self, factor: float = 3.0
    ) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """
        Optional:
        ...
        for index, row in self._outlier_result.iterrows():
            for time_point in timestamps[
                timestamps.index(int(row["start_time"])) : timestamps.index(
                    int(row["end_time"])
                )
                + 1
            ]:
    ...
    def calc_statistic_outliers(
        ...
        for t1 in range(len(timestamps) - 1):
            for t2 in range(t1 + 1, len(timestamps)):
                ...
                subsequence_ratings = subsequence_ratings.assign(outlier=0)
                std_dev = np.std(
                    subsequence_ratings[subsequence_ratings["rating"] >= 0][
                        "rating"
                    ].values
                )
                mean = np.mean(
                ...
                subsequence_ratings = subsequence_ratings.assign(
                    distance=lambda x: abs(x["rating"] - mean)
                )
                subsequence_ratings[subsequence_ratings["rating"] >= 0] = (
                    subsequence_ratings[subsequence_ratings["rating"] >= 0].assign(
                        outlier=lambda x: (x["distance"] <= factor * std_dev) - 1
                    )
                )
                self._outlier_rating = self._outlier_rating.append(subsequence_ratings)
