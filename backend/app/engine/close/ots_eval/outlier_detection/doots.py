...
class DOOTS(object):
    ...
    def __init__(
        self, data: pandas.DataFrame, weighting: bool = False, jaccard: bool = False
    ):
        """
        Params:
    ...
    def mark_outliers(self, tau: float) -> Tuple[pandas.DataFrame, pandas.DataFrame]:
        ...
        self._outlier_result = self._outlier_rating[
            (self._outlier_rating["distance"] >= tau)
        ...
        for index, row in conseq_outliers.iterrows():
            for time_point in time_points[
                ...
                if self._data.loc[
                    ...
                    self._data.loc[
                        (self._data[self._time_column_name] == int(time_point))
                        & (
                            self._data[self._object_column_name]
                            == row[self._object_column_name]
                        ),
    ...
    def calc_object_rating(
        self, ids_to_rate: list, end_time: int, start_time: int = None
    ) -> dict:
        """
        Params:
        ...
        for id in ids_to_rate:
            ...
            if (
                len(
                    cur_group[cur_group[self._time_column_name] == end_time][
                ...
                # Add the proportion of clusters before last timestamp, that merged in last cluster
                    ...
                    if int(cluster) < 0:
                        ...
                        object_ratings.append(
                            self._cluster_compositions[last_cluster][cluster]
                        )
                    num_clusters += 1
    ...
    def calc_outlier_rating(self) -> pandas.DataFrame:
        ...
        for i in range(0, len(timestamps) - 1):
            for j in range(i + 1, len(timestamps)):
                time_ratings = self.rate_object(
                    start_time=timestamps[i], end_time=timestamps[j]
                )
                for object in time_ratings:
                    cluster = self._data[
                        (self._data[self._object_column_name] == object)
        ...
        outlier_rating = pandas.DataFrame(
            ratings,
            columns=[
