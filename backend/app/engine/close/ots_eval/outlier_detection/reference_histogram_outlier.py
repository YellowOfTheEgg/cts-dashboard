...
class HistOutlier:
    ...
    @staticmethod
    def calc_outlier_degree(
        rating: pd.DataFrame, clusters: pd.DataFrame
    ) -> pd.DataFrame:
        """
        Params:
        ...
        result = pd.DataFrame()
        ref_sequence_clusters = rating.groupby(
            ["start_time", "end_time", "cluster_end_time"]
        ).max()

        for index, row in ref_sequence_clusters.iterrows():
            sequence_cluster_distance = rating[
                (rating["start_time"] == index[0])
                & (rating["end_time"] == index[1])
                & (rating["cluster_end_time"] == index[2])
            ]
            sequence_cluster_distance = sequence_cluster_distance.assign(
                distance=lambda x: row["rating"] - x["rating"]
            )
            ...
            for time_point in times:
                ...
                if time_point in object_times:
                    ...
                    if times.index(time_point) + 1 < len(times):
                        ...
                        if (
                            followup_timepoint in object_times
                        ) and time_point in object_times:
                            # check if the object is an outlier at both timepoints:
                            current_outlier = clusters[
                                (clusters[time_column] == time_point)
                                & (clusters[object_column] == object)
                            ][cluster_column].tolist()[0]
                            followup_outlier = clusters[
                                (clusters[time_column] == followup_timepoint)
                                & (clusters[object_column] == object)
                            ][cluster_column].tolist()[0]
