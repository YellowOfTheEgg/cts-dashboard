from app.engine.close.ots_eval.outlier_detection.dact import DACT
...
# dact requires specific input format
...
class Dact:
    def __init__(self, dataset, settings_dataset, settings_dact):
        self.dataset = dataset
        self.settings_dataset = settings_dataset
        self.settings_dact = settings_dact
    ...
    def run(self):
        ...
        detector = DACT(restructured_df)

        details = detector.calc_outlier_degree()
        outlier_result, details = detector.mark_outliers(tau=self.settings_dact.tau)
        outlier_result.drop(outlier_result.columns[2], inplace=True, axis=1)
        # a=outlier_result.query('outlier<0')
