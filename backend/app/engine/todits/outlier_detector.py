...
class OutlierDetector:
    ...
    def mark_preoutliers(self, marked_outliers):
        ...
        outliers = marked_outliers.query("outlier==True").drop(
            ["cluster_id", "count", "outlier"], axis=1, errors="ignore"
        )

        preoutliers = outliers.apply(lambda x: get_preoutlier(x), axis=1)
        preoutliers["preoutlier"] = True

        marked_preoutliers = marked_outliers.merge(
            preoutliers, on=["object_id", "time"], how="left"
        )
        marked_preoutliers["preoutlier"] = [
            False if preout is np.nan else True
            for preout in marked_preoutliers["preoutlier"]
        ]
